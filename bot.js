...
const PATH_GAMBLE = BASE + "gamble.json"; // 도박 잔고
const PATH_QUIZ_BANK = BASE + "quiz_bank.json"; // 퀴즈 문제 풀
...
let gambleData = readJson(PATH_GAMBLE, {}); // {id: {name, balance}}
let quizBank = readJson(PATH_QUIZ_BANK, []); // [{question, answer}]
...
let quizPool = {}; // {room: [{question, answer, hint}]}

function ensureQuizBank() {
  if (!quizBank || !(quizBank instanceof Array) || quizBank.length === 0) {
    quizBank = BASE_QUIZ.slice();
    writeJson(PATH_QUIZ_BANK, quizBank);
  }
}
...
const PENALTIES = [
  "요즘 설레는 일이 있다? (O/X)",
  "연애에서 먼저 고백하는 편이다? (O/X)",
  "이성에게 중요한 건 대화다? (O/X)",
  "첫인상에서 가장 보는 것은? 1)눈빛 2)말투 3)웃음",
  "데이트 선호는? 1)집 2)야외 3)여행"
];

function ensureQuizPool(room) {
  ensureQuizBank();
  if (!quizPool[room] || quizPool[room].length === 0) {
    let source = quizBank && quizBank instanceof Array && quizBank.length > 0 ? quizBank.slice() : BASE_QUIZ.slice();
    shuffleArray(source);
    quizPool[room] = source;
  }
}

function getQuizFromPool(room) {
  ensureQuizPool(room);
  if (quizPool[room].length === 0) {
    return null;
  }
  let item = quizPool[room].pop();
  return { question: item.question, answer: item.answer, hint: "초성: " + getInitials(item.answer) };
}

function getLocalQuizQuestion(room) {
  return getQuizFromPool(room || "global");
}

function fetchQuizQuestion(room) {
  return getLocalQuizQuestion(room);
}
...
function startQuiz(room) {
  ...
  let quiz = fetchQuizQuestion(room);
  if (!quiz) {
  ...
  if (quiz.options && quiz.options.length > 0) {
    ...
    lines.push("정답은 텍스트로 입력하세요.");
  }
  lines.push("⏳ 제한 시간 10초");
...
function renderQuizRank() {
  ...
  let lines = [];
  for (let i = 0; i < top.length; i++) {
    lines.push((i + 1) + "위 " + top[i].name + " - " + top[i].score + "점");
  }
...
function ensureGamble(msg) {
  ...
  let name = msg.author && msg.author.name ? msg.author.name : "";
  if (!gambleData[id]) {
    gambleData[id] = { name: name, balance: 1000 };
    writeJson(PATH_GAMBLE, gambleData);
  } else {
    gambleData[id].name = name;
  }
}

function renderGambleBalance(msg) {
  ensureGamble(msg);
  let id = getUserId(msg);
  let info = gambleData[id];
  return formatBlock("도박", ["닉네임: " + info.name, "잔고: " + info.balance + "원"]);
}

function gamblePlay(msg, amount) {
  ensureGamble(msg);
  let id = getUserId(msg);
  let info = gambleData[id];
  if (amount <= 0) {
    return formatBlock("도박", ["베팅 금액을 1 이상 입력하세요."]);
  }
  if (amount > info.balance) {
    return formatBlock("도박", ["잔고 부족", "현재 잔고: " + info.balance + "원"]);
  }
  ...
  writeJson(PATH_GAMBLE, gambleData);
  let lines = [];
  lines.push(win ? "🎉 베팅 성공! +" + amount + "원" : "💥 베팅 실패 -" + amount + "원");
  lines.push("현재 잔고: " + info.balance + "원");
  return formatBlock("도박", lines);
}

function gambleAllIn(msg) {
  ensureGamble(msg);
  let id = getUserId(msg);
  let info = gambleData[id];
  if (info.balance <= 0) {
    return formatBlock("도박", ["올인할 잔고가 없습니다."]);
  }
  ...
  try {
    ...
    if (quizState[msg.room]) {
      ...
      if (expiredAnswer !== null) {
        msg.reply(formatBlock("퀴즈", ["⏰ 시간 종료", "정답: " + expiredAnswer, "!퀴즈 로 다시 시작하세요."]));
        return;
      }
      ...
      if (numberMatch || normalizeAnswer(input) === normalizeAnswer(answer)) {
        addQuizScore(msg);
        msg.reply(formatBlock("퀴즈", ["🎉 정답!", quizState[msg.room].question, "정답: " + answer]));
...
function onCommand(cmd) {
  try {
    ...
    if (cmd.command === "퀴즈초기화") {
      if (!isAdmin(cmd)) {
        cmd.reply(formatBlock("퀴즈", ["초기화 권한이 없습니다."]));
        return;
      }
      quizRank = {};
      writeJson(PATH_QUIZ, quizRank);
      cmd.reply(formatBlock("퀴즈", ["퀴즈 랭킹을 초기화했습니다."]));
      return;
    }
    ...
    if (cmd.command === "도박") {
      ...
      if (args[0] === "올인") {
        cmd.reply(gambleAllIn(cmd));
        return;
      }
      let amount = parseInt(args[0], 10);
      if (isNaN(amount)) {
        cmd.reply("베팅 금액을 숫자로 입력하세요.");
        return;
      }
      cmd.reply(gamblePlay(cmd, amount));
      return;
    }
  } catch (error) {
